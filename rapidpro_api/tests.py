...
class RapidproApiViewTests(APITestCase):
    ...
    def test_profileSync_language_fields(self):
        ...
        url = "{}?turn_field=language&rp_field=language".format(
    ...
    def test_profileSync_required_fields(self):
        ...
        url = "{}?turn_field=language".format(
            reverse("profileSync-profileSync", args=[connection.pk])
        )
        response = self.client.post(url, {}, format="json")
        self.assertEqual(response.data["rp_field"], "This query parameter is required.")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
